...
void DrawFissionEE(TFile *f, string FC, Int_t ch, Bool_t sc, Bool_t wcs)
{ /// Draw a single E vs E(t) 2D histogam.
  /// Choose fission chamber, channel, total/scattered, cross section weighting.
  /// Beautiful color map, histogram label

    string IntPath;
    if (sc) IntPath = FC+"/EToFvsEkin/Scattered/"+FC+"_EToFvsEkin_Sc_Ch."+std::to_string(ch+1);
    else    IntPath = FC+"/EToFvsEkin/"+FC+"_EToFvsEkin_Ch."+std::to_string(ch+1);
    TH2F *h2 = (TH2F*)f->Get(IntPath.c_str());

    // weight with cross section
    if (wcs) h2 = WeightWithXS(h2, "~/TrackLength/FissionXS.root", FC, "JEFF_3.3");
    ...
    string text = FC+", ch."+std::to_string(ch+1)+", sc";
    if (!sc) text = text + "+unsc";
...
void drawTL()
{
    ...
    gROOT->ForceStyle(kTRUE);
    gStyle->SetLegendFont(132);
//    TFile *f = TFile::Open("~/TrackLength/G4UFCvsH19_1E9_hist.root", "READ");
    TFile *g = TFile::Open("~/TrackLength/G4UFCvsH19_23082016_hist.root");
//    string StrFileInPath = "/home/hoffma93/bigdata/nELBE/G4TransMis/20200131/G4UFCvsH19_31012020.root";
//    string StrFileInPath = "/home/hoffma93/TrackLength/G4UFCvsH19_1E7_tree.root";
    string File3 = "/home/hoffma93/TrackLength/G4UFCvsH19_1E8_hist_result.root";
    TFile *f = TFile::Open(File3.c_str(), "READ");
//    DrawWeight(File3);
//    DrawWspectrum(StrFileInPath, "UFC", 1);
//    DrawWspectra(StrFileInPath);
//    DrawRefH19(File3, 7);

//    TFile *f = TFile::Open("/home/hoffma93/TrackLength/G4UFCvsH19_1E6_fast_Phi.root", "READ");
//    TFile *g = TFile::Open("/home/hoffma93/TrackLength/G4UFCvsH19_1E6_fast_Theta.root", "READ");
//    DrawWspectra(f, g, "UFC", 0, 1);
